...
- **Turn loop**: screenshot → RAM reads → API call → tool execution → repeat. `TurnContextBuilder` in `agent/turn_context.py` assembles context (spatial, battle, party, stuck warnings, etc.). Map transition logic extracted to `_handle_map_transition()`. **Stats dataclasses**: `_PeriodicStats` (16 fields, `reset()` method) and `_TurnTokens` (5 fields) replace flat `self._stats_*` / `self._last_turn_*` attributes. **Named thresholds** at module top: `_SCREEN_CHANGE_THRESHOLD`, `_PP_HEAL_INJECT/REMOVE`, `_NAV_NONCOMPLIANCE_LIMIT`, `_MIN_POSITIONS_FOR_NAV`, `_MAX_TACTICAL_LEN`, etc. `GameState.check_tactical_override_expiry()` encapsulates private `_tactical_goal_override`/`_tactical_override_grace` access.
...
- **World map** (`world_map.py`): persistent tile accumulator + A* + **map connectivity graph** (`map_graph`) for BFS hop routing. **Thread-safe**: all mutable state protected by `threading.RLock`; public methods acquire the lock, internal `_*_unlocked` variants used for internal calls. Main thread writes (`update`, `ensure_graph_edge`, `record_warp_transition`), analysis thread reads (`render_summary`, `frontier_ratio`, `find_nav_hint`). `ensure_graph_edge()` guarantees bidirectional edges on every map transition. **Tile protection**: `update()` never downgrades a walkable tile to `#` — shelf-collapse in `spatial_context.py` is viewport-relative and can produce false walls at viewport edges; once a tile is observed walkable it stays walkable (game map is static). **Pair-edge protection**: only accumulates `pair_blocked_edges` where both endpoints are confirmed walkable (edges involving walls are redundant noise). **Pathfinding variance** (0–3): cost jitter forces alternate routes when stuck; escalated from `stuck_count` in `turn_context.py`. **Route cache**: paths confirmed by successful warp transition reused via `get_cached_route()`; skipped when variance>0. **Warp cycling**: detected after 3+2 transitions in both directions; cycling pair added to `_cycling_maps` (excluded from BFS); decays after 30 turns. **Frontier-first**: when `frontier_ratio() > 0.5` and not stuck, NAV routes to nearest frontier tile (biased toward high-density clusters) before goal-directed routing. **Exploration nudge**: `turn_context.py` injects `⚑ EXPLORE` message when frontier_ratio > 0.5 and stuck_count < 5. **Map markers**: `place_marker`/`remove_marker` tools; max 8/map; rendered `*` with legend; injected into spatial text each turn. Persisted in `world_map.json`.

- **NAV pipeline** (`nav_planner.py`, entry `compute_nav()`): returns `NavResult(spatial_text, nav_method, nav_suggestion)` NamedTuple (no module globals). Stored on `TurnContextBuilder._last_nav_result`; `game_agent.py` reads it for compliance logging. Pipeline: (0) frontier-first, (0b) route cache, (1) map graph BFS → A\* to next-hop warp (retries up to 3× excluding failed first hops), (2a) frontier fallback when all hops exhausted or 2 consecutive "exhausted" results, (2b) COMPASS from goal text, (3) frontier exploration A\*, (4) ledge-aware path truncation.

- **Warp dest_name overrides** (`warp_overrides.py`): disambiguates caves with multiple warps to same `dest_map`. When adding overrides: pokered ASM uses **1-based** warp indices; RAM uses **0-based**. **MAP_HINTS style**: vague directional nudges for overworld ("There's a town to the north"), floor progression for dungeons ("This cave has multiple floors. Explore to find stairs."). Never include warp coords or step-by-step routing. STORY_PROGRESSION goals must contain a recognizable map name for NAV target extraction. Detailed warp/path info belongs in location notes files.

- **NAV compliance logging**: `game_agent.py` compares the first directional token of the model's action vs NAV's suggestion (from `NavResult.nav_suggestion`) and logs `NAV COMPLIANCE: followed=true/false`. Thresholds: `_NAV_NONCOMPLIANCE_LIMIT` (3) consecutive divergences + `_NAV_GUARD_MIN_MAP_TURNS` (8) triggers forced NAV step.
...
Sections: `<notation>`, `<spatial_context>`, `<navigation>` (exploration-first framing: explores before routing; COMPASS warning prevents compass bearings being converted to frame counts like "6 LEFT" → `L96`; ⚑ EXPLORE nudge explained), `<battle_context>`, `<menu_context>`, `<authority>`, `<memory>`.
