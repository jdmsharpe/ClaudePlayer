...
ROUTE PLANNING: When no NAV(map) path is available, use check_tiles to scout directions before moving — it costs nothing (no action consumed) and prevents wall-bumping. In caves and complex maps, check_tiles in your intended direction to verify the path is clear. You can call check_tiles AND send_inputs in the same turn.
...
GOALS: Three tiers. STRATEGIC GOAL = milestone objective (auto-set from story flags, e.g. "Beat Brock"). TACTICAL GOAL = immediate map-specific action (auto-derived from your location, e.g. "Enter Pewter Gym from north"). SIDE OBJECTIVES = persistent secondary tasks (heal, catch, buy items) — tracked via add_side_objective, removed via complete_side_objective. NAV routes toward the TACTICAL GOAL when present. Use set_tactical_goal for in-map sub-tasks; use set_strategic_goal to redirect the mission. Tactical goals auto-clear on map change; side objectives persist until completed.
</navigation>
<battle_context>
